...
# ── classification config (edit classification_config.py to change HOT rules) ─
from classification_config import (
    THERAPEUTIC_AREAS,
    HOT_CLASSIFICATION_PROMPT,
...
def classify_news_type(text):
    """Fallback keyword-based news-type classifier (used when AI is disabled)."""
    t = (text or "").lower()
...
def classify_hot(text):
    """Fallback keyword-based HOT classifier (used when AI is disabled)."""
    return "HOT" if _contains_any(text, HOT_KEYWORDS) else "NON-HOT"


# ─────────────────────────────────────────────────────────────────────────────
# IMPROVED DEDUPLICATION
# Uses company + headline similarity + URL domain + date
# instead of a simple exact-headline key.
# ─────────────────────────────────────────────────────────────────────────────
def _normalise_for_dedup(text):
    """Lower-case, collapse spaces, strip punctuation for similarity comparison."""
    t = re.sub(r"[^a-z0-9\s]", " ", (text or "").lower())
    return re.sub(r"\s+", " ", t).strip()

def _headline_similar(h1, h2, threshold=0.72):
    """Return True if two normalised headlines are very similar (Jaccard on trigrams)."""
    if not h1 or not h2:
        return False
    def trigrams(s):
        return {s[i:i+3] for i in range(len(s)-2)} if len(s) >= 3 else {s}
    t1, t2 = trigrams(h1), trigrams(h2)
    if not t1 or not t2:
        return h1 == h2
...
def _url_domain(url):
    try:
        p = urlparse(url or "")
        parts = p.netloc.lower().split(".")
        return ".".join(parts[-2:]) if len(parts) >= 2 else p.netloc.lower()
    except Exception:
        return ""
...
def deduplicate(records):
    ...
    Two articles are duplicates when they share the same company AND
    (a) normalised headline Jaccard similarity ≥ 0.72, OR
    (b) same URL domain + date + first-60-chars of normalised headline.
    Prefers primary company PR/newsroom sources over aggregators.
    """
    # Score sources: lower = preferred
    def _source_priority(r):
        st = (r.get("source_type") or "").lower()
        if "pr" in st or "newsroom" in st:
            return 0
        if "google" in st:
            return 2
        if "ai" in st:
    ...
    ordered = sorted(records, key=_source_priority)

    out = []
    for rec in ordered:
        norm_h  = _normalise_for_dedup(rec.get("headline", ""))
        company = (rec.get("company") or "").lower().strip()
        domain  = _url_domain(rec.get("url", ""))
        art_date= (rec.get("date") or "")[:10]

        duplicate = False
        for kept in out:
            kc = (kept.get("company") or "").lower().strip()
            if company and kc and company != kc:
                continue   # different company → not a dup
            kh = _normalise_for_dedup(kept.get("headline", ""))
            # Check headline similarity
            if _headline_similar(norm_h, kh):
                duplicate = True
                break
            # Check domain+date+short-headline
            kd = _url_domain(kept.get("url", ""))
            if domain and kd and domain == kd and art_date == (kept.get("date") or "")[:10]:
                if norm_h[:60] == kh[:60]:
                    duplicate = True
                    break
...
# ══════════════════════════════════════════════════════════════════════════════
# LLM-BASED HOT / NON-HOT CLASSIFIER
# ══════════════════════════════════════════════════════════════════════════════

def _validate_classification(result: dict) -> dict:
    """Validate and normalise a single classification dict from the LLM."""
    classification = str(result.get("classification", "NON-HOT")).strip()
    if classification not in ALLOWED_CLASSIFICATIONS:
        classification = "NON-HOT"

    news_type = str(result.get("news_type", "Other")).strip()
    if news_type not in ALLOWED_NEWS_TYPES:
        news_type = "Other"

    confidence = str(result.get("confidence", "Review Recommended")).strip()
    if confidence not in ALLOWED_CONFIDENCE:
        confidence = "Review Recommended"

    rationale = str(result.get("rationale", "")).strip()[:400]

    ta_raw = result.get("therapeutic_areas", [])
    if not isinstance(ta_raw, list):
        ta_raw = []
    therapeutic_areas = [t for t in ta_raw if t in THERAPEUTIC_AREAS]
...
def _parse_hot_json(text: str) -> Optional[dict]:
    """Extract a JSON object from the LLM's HOT classification response."""
    text = re.sub(r"^```(?:json)?", "", text.strip(), flags=re.I)
    text = re.sub(r"```$", "", text.strip())
    # Try direct parse
    try:
...
def _classify_article_hot(
    ...
    """
    Call the LLM to classify a single article as HOT / NON-HOT / EXCLUDED.
    Returns a validated classification dict.
    Falls back to NON-HOT with Review Recommended on any error.
    """
    fallback = {
        "classification": "NON-HOT",
        "news_type": "Other",
        "confidence": "Review Recommended",
        "rationale": "Classification could not be completed — LLM call failed.",
        "therapeutic_areas": [],
    }
    if not api_key:
        return fallback
    ...
    if article_body:
        context += f"\nArticle excerpt: {article_body[:1000]}"
    ...
    try:
        resp = requests.post(
            ...
            json={
                ...
                "response_format": {"type": "json_object"},
                "messages": [
                    {"role": "system", "content": HOT_CLASSIFICATION_PROMPT},
        ...
        obj = _parse_hot_json(raw)
        if obj is None:
            logger.warning(f"[HOT] Could not parse JSON for: {headline[:60]}")
            return fallback
        return _validate_classification(obj)
    except Exception as exc:
        logger.warning(f"[HOT] LLM error for '{headline[:60]}': {exc}")
        return fallback


def classify_articles_hot_batch(records, batch_size=10):
    """
    Classify every record as HOT / NON-HOT / EXCLUDED using the LLM.
    Adds keys: hot, news_type, hot_confidence, hot_rationale, therapeutic_areas.
    EXCLUDED articles are flagged but NOT removed here — the caller decides.
    Falls back to keyword-only logic when GROQ_API_KEY is not set.
    """
    api_key = os.getenv("GROQ_API_KEY")
    model   = os.getenv("GROQ_MODEL", DEFAULT_MODEL)

    if not api_key:
        _log("[HOT] GROQ_API_KEY not set — using keyword-only HOT classification")
        for r in records:
            r["hot"]              = classify_hot(r.get("headline",""))
            r["news_type"]        = classify_news_type(r.get("headline",""))
            r["hot_confidence"]   = "Review Recommended"
            r["hot_rationale"]    = "Keyword-only (AI not available)"
            r["therapeutic_areas"]= ""
        return records

    _log(f"\n── LLM HOT Classification ({model}) — {len(records)} articles ──")

    for idx, rec in enumerate(records, 1):
        _log(f"  [HOT] {idx}/{len(records)} — {rec.get('headline','')[:70]}")
        ...
        rec["hot"]               = result["classification"]   # HOT / NON-HOT / EXCLUDED
        rec["news_type"]         = result["news_type"]
        rec["hot_confidence"]    = result["confidence"]
        rec["hot_rationale"]     = result["rationale"]
        rec["therapeutic_areas"] = "; ".join(result["therapeutic_areas"])

        # Small sleep to respect rate limits
        if idx % batch_size == 0:
            time.sleep(1)

    hot_count      = sum(1 for r in records if r.get("hot") == "HOT")
    excluded_count = sum(1 for r in records if r.get("hot") == "EXCLUDED")
    _log(f"[HOT] Done — HOT:{hot_count}  NON-HOT:{len(records)-hot_count-excluded_count}  EXCLUDED:{excluded_count}")
    return records
...
COLUMNS = [
    ...
    ("Therapeutic Area",   "therapeutic_areas",  28),
    ("HOT / NON-HOT",      "hot",                14),
    ("News Type",          "news_type",          20),
    ("Confidence",         "hot_confidence",     20),
    ("Rationale",          "hot_rationale",      50),
...
def _write_sheet(wb, title, records, fill_color_override=None):
    """Write a list of records to a new worksheet with standard formatting."""
    ...
    for i, rec in enumerate(records):
        ...
        if fill_color_override:
            fill = PatternFill("solid", fgColor=fill_color_override)
        elif is_hot:
            fill = PatternFill("solid", fgColor=HOT_YELLOW)
        else:
            fill = PatternFill("solid", fgColor=ALT_GREY) if alt else None
        ...
        for ci, (_, key, _) in enumerate(COLUMNS, 1):
            ...
            if key == "url" and val:
                ...
                cell.font = Font(name="Calibri", size=10, bold=(is_hot and key in ("hot","HOT / NON-HOT")))
...
def export_to_excel(records, start_date, end_date, excluded_records=None):
    """
    Export results to Excel with separate worksheets:
    - HOT News
    - NON-HOT News
    - Excluded (debug)
    - All News (combined, for backwards compat)
    - Summary
    """
    ...
    hot_records    = [r for r in records if r.get("hot") == "HOT"]
    nothot_records = [r for r in records if r.get("hot") == "NON-HOT"]
    excl_records   = list(excluded_records or []) + [r for r in records if r.get("hot") == "EXCLUDED"]

    wb = Workbook()
    # First sheet = HOT News
    ws_hot = wb.active
    ws_hot.title = "HOT News"
    ...
    # Write HOT sheet directly (active sheet)
    for ci, (label, _, width) in enumerate(COLUMNS, 1):
        c = ws_hot.cell(row=1, column=ci, value=label)
        c.font, c.fill, c.alignment, c.border = hf, hfill, ha, _BORDER
        ws_hot.column_dimensions[get_column_letter(ci)].width = width
    ws_hot.row_dimensions[1].height = 32
    ws_hot.freeze_panes = "A2"
    for i, rec in enumerate(hot_records):
        fill = PatternFill("solid", fgColor=HOT_YELLOW)
        for ci, (_, key, _) in enumerate(COLUMNS, 1):
            val  = rec.get(key, "") or (rec.get("company","") if key == "screened_entity" else "")
            cell = ws_hot.cell(row=i+2, column=ci, value=val)
            cell.alignment, cell.border = ra, _BORDER
            cell.fill = fill
            if key == "url" and val:
                cell.hyperlink = val
                ...
                cell.font = Font(name="Calibri", size=10, bold=(key == "hot"))
        ws_hot.row_dimensions[i+2].height = 45
    if hot_records:
        ws_hot.auto_filter.ref = f"A1:{get_column_letter(len(COLUMNS))}{len(hot_records)+1}"

    # NON-HOT sheet
    _write_sheet(wb, "NON-HOT News", nothot_records)

    # Excluded / debug sheet
    if excl_records:
        _write_sheet(wb, "Excluded", excl_records, fill_color_override="E2EFDA")

    # All News (combined, sorted by date desc, for backwards compat)
    all_sorted = sorted(records, key=lambda r: r.get("date",""), reverse=True)
    _write_sheet(wb, "All News", all_sorted)
    ...
    ss["A6"], ss["B6"] = "HOT Items",        len(hot_records)
    ss["A7"], ss["B7"] = "NON-HOT Items",    len(nothot_records)
    ss["A8"], ss["B8"] = "Excluded Items",   len(excl_records)
    ss["A9"], ss["B9"] = "Product Results",  sum(1 for r in records if r.get("screening_category")=="Product Screening")
    for row in range(3, 10):
        ss.cell(row=row, column=1).font = bf

    ss["A11"], ss["B11"] = "News Type", "Count"
    ss["A11"].font = ss["B11"].font = bf
    counts = {}
    for r in records:
        nt = r.get("news_type","Other")
        counts[nt] = counts.get(nt,0) + 1
    for ri, (nt, c) in enumerate(sorted(counts.items(), key=lambda x:-x[1]), 12):
        ss.cell(row=ri, column=1, value=nt)
        ss.cell(row=ri, column=2, value=c)

    # Therapeutic area breakdown
    next_row = 12 + len(counts) + 2
    ss.cell(row=next_row, column=1, value="Therapeutic Area").font = bf
    ss.cell(row=next_row, column=2, value="Count").font = bf
    ta_counts: Dict[str, int] = {}
    for r in records:
        for ta in (r.get("therapeutic_areas") or "").split(";"):
            ta = ta.strip()
            if ta:
                ta_counts[ta] = ta_counts.get(ta, 0) + 1
    for ri, (ta, c) in enumerate(sorted(ta_counts.items(), key=lambda x:-x[1]), next_row+1):
        ss.cell(row=ri, column=1, value=ta)
        ss.cell(row=ri, column=2, value=c)
...
def run(start_date, end_date, use_ai=True, use_google=True, use_pr=True, use_products=True, include_undated=True):
    ...
    _log(f"\n── LLM HOT/NON-HOT Classification ──")
    today_str = datetime.now().strftime("%Y-%m-%d")

    # Assign serial numbers and date_collected before classification
    for i, r in enumerate(deduped, 1):
        r["serial_number"]  = i
        r["date_collected"] = today_str
        # Initialise classification fields (overwritten by LLM below)
        r.setdefault("hot",            "NON-HOT")
        r.setdefault("news_type",      "Other")
        r.setdefault("hot_confidence", "Review Recommended")
        r.setdefault("hot_rationale",  "")
        r.setdefault("therapeutic_areas", "")

    classified = classify_articles_hot_batch(deduped)

    # Split off EXCLUDED records (not relevant to therapeutic scope)
    excluded_records  = [r for r in classified if r.get("hot") == "EXCLUDED"]
    retained_records  = [r for r in classified if r.get("hot") != "EXCLUDED"]
    hot_count         = sum(1 for r in retained_records if r.get("hot") == "HOT")
    _log(f"  Retained: {len(retained_records)}  HOT: {hot_count}  Excluded: {len(excluded_records)}")

    # Export Excel
    _log(f"\n── Export ──")
    excel_path = export_to_excel(retained_records, start_date, end_date, excluded_records=excluded_records)

    # Swap deduped → retained for summary stats below
    deduped = retained_records
